    ...
    stages {
        ...
        stage('Push to Central Hub Container Registry') {
            steps {
                ...
                sh 'docker login -u "$DOCKER_HUB_CREDS_USR" -p "$DOCKER_HUB_CREDS_PSW" && docker tag "${IMAGE_TAG}" "docker.io/${DOCKER_HUB_CREDS_USR}/devops-pipeline-master-suite:latest" && docker push "docker.io/${DOCKER_HUB_CREDS_USR}/devops-pipeline-master-suite:latest"'
            }
        }
